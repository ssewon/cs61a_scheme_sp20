...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(operation)
        remaining = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(operation, remaining, env)
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        more_values = scheme_eval(bindings.first.rest.first, env)
        names = Pair(bindings.first.first, names)
        validate_formals(names)
        values = Pair(more_values, values)
